...
全屏
fullPage.addEventListener("click",function (ev) {
...
取消全屏
backPage.addEventListener("click",function (ev) {
...
#####css3 中 在全屏操作后 自带的控制栏会显示 在显示的时候隐藏
...
+ 上传文件预览图片
  +  获取到文件中的file 文件域
  + 监听onchange事件  获取到FileReader() 对象
  + 调用readAsDatURL(this.files[0])   
  + 调用onload方法 加载完毕方法 在里面可以调用this.result 就是返回的图片路径 是base64位格式的 可以表示图片
